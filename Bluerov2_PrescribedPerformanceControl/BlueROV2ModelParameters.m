...
function modelParameters = BlueROV2ModelParameters(nu, eta)
% BlueROV2ModelParameters: ROV 6DOF model using CRB + CA in skew-form (Fossen)
%
% Inputs:
%   nu  : [6x1] vector [u; v; w; p; q; r]
%   eta : [6x1] vector [x; y; z; phi; theta; psi]
%
% Output:
%   modelParameters : struct with Mij, Cij, Dij, gi blocks

% === Parameters (BlueROV2, Table A1) ===
params = struct( ...
    'm',       13.5, ...
    'xg',      0.0,  'yg', 0.0,  'zg', 0.0, ...
    'Ixx',     0.26, 'Iyy', 0.23, 'Izz', 0.37, ...
...
% === Coriolis rigid-body in skew form ===
CRB = [ zeros(3),    -m*S(nu_2);
       -m*S(nu_2), -S(I_CG*nu_2) ];

% === Hydrodynamic Coriolis (Added-mass) via alfas/betas ===
MA11 = MA(1:3,1:3); MA12 = MA(1:3,4:6);
MA21 = MA(4:6,1:3); MA22 = MA(4:6,4:6);

% Definición explícita de alfas y betas
alpha_vec = MA11*nu_1 + MA12*nu_2;    % [alpha1; alpha2; alpha3]
beta_vec  = MA21*nu_1 + MA22*nu_2;    % [beta1;  beta2;  beta3]

alpha1 = alpha_vec(1); alpha2 = alpha_vec(2); alpha3 = alpha_vec(3);
beta1  = beta_vec(1);  beta2  = beta_vec(2);  beta3  = beta_vec(3);

% Construcción de CA usando S(·)
CA = [ zeros(3),            -S([alpha1; alpha2; alpha3]);
      -S([alpha1; alpha2; alpha3]), -S([beta1;  beta2;  beta3]) ];
...
% === Damping (linear + cuadrático, definido positivo) ===
D_L = diag([params.X_u, params.Y_v, params.Z_w, ...
            params.K_p, params.M_q, params.N_r]);
...
phi = eta(4); theta = eta(5);
g = m * 9.81;
B = g * 1.00;   % neutro (ajusta >1 para flotabilidad positiva)
...
% (Opcional: devuelve también alfas/betas para depuración)
modelParameters.alpha = alpha_vec;
modelParameters.beta  = beta_vec;
end
